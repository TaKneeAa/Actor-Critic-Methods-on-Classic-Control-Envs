...
class MDP():
    ...
    def update_policy(self, observations, actions, product_terms, policy_optimizer, policy_std_optimizer, iter, sigma_delay):
        policy_optimizer.zero_grad()
        policy_std_optimizer.zero_grad()
    ...
    def run_vanilla_policy_gradients(self, num_iters, num_value_optim_steps, batch_size = 4000, product_terms_type = 'discounted_returns', model_name = 'default', policy_lr = 1e-4, value_fn_lr = 1e-3, normalize_gae = True, sigma_delay = 3000, sigma_lr = 1e-4):
        """
        ...
        policy_optimizer = optim.Adam(self.policy.action_mean_nn.parameters(), lr=policy_lr)
        policy_std_optimizer = optim.Adam([self.policy.log_std], lr=sigma_lr)
        ...
        for iter in range(num_iters):
            ...
            policy_loss_numpy = self.update_policy(observations, actions, product_terms, policy_optimizer, policy_std_optimizer, iter, sigma_delay)
...
def run_vpg(args):
    ...
    config.sigma_delay = args.sigma_delay
    config.sigma_lr = args.sigma_lr
    ...
    mdp.run_vanilla_policy_gradients(num_iters = args.num_iters, num_value_optim_steps = args.num_value_optim_steps, batch_size = args.batch_size, product_terms_type = args.product_terms_type, model_name = model_name, value_fn_lr = args.value_fn_lr, policy_lr = args.policy_lr, normalize_gae = args.normalize_gae, sigma_delay = args.sigma_delay, sigma_lr = args.sigma_lr)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sigma_delay', type=int, default=3000)
    parser.add_argument('--sigma_lr', type=int, default=1e-4)
