...
from p_utils import PolicyNet,QNet
import copy
import gym
import numpy as np
...
wandb.init(project="Actor-Critics Methods for Continous Action spaces",  name="Pendulum-DDPG")
...
pi_optimizer = Adam(actor_net.parameters(), lr=0.0005)
...
while(True):
    ...
    while(done!=True):
        ...
        if ctr>10000:
            a = actor_net(torch.from_numpy(s.astype(np.float32))).detach().numpy() + 0.1 * np.random.randn(ndim_actions)
        else:
        ...
        rew+=r
    rew_arr.append(rew)
    wandb.log({"Pendulum-v1 Reward Per Episode" :rew},step=ctr)
    ...
    if(ctr%25 ==0):
        ...
        while(K<10):
            ...
            #Compute Target
            with torch.no_grad():
                y  = [b[2]+gamma*critic_net_target(torch.from_numpy(b[3].astype(np.float32)),actor_net_target(torch.from_numpy(b[3].astype(np.float32)))) for b in B]
            ...
            wandb.log({"Pendulum Q-network error":del_Q},step=ctr)
            ...
            wandb.log({"Pendulum Pi-network error":del_pi},step=ctr)
    ...
    if (ctr%5000 == 0 ):
        ...
        loss_file = open('./checkpoints/Pendulum/'+str(run_num)+"reward.txt", "a")
        pi_file = open('./checkpoints/Pendulum/'+str(run_num)+"pi.txt", "a")
        q_file = open('./checkpoints/Pendulum/'+str(run_num)+"q.txt", "a")
